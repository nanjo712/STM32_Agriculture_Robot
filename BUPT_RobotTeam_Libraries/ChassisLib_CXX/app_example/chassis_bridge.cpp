...
//            dji_PosCtrl(ControllMsg.boardId,ControllMsg.motorId,ControllMsg.pos);
            DJIBoard_PosCtrl(&hcan1,ControllMsg.boardId,ControllMsg.motorId,ControllMsg.pos);
            break;
        case Mode_VelCtrl:
            DJIBoard_VelCtrl(&hcan1,ControllMsg.boardId,ControllMsg.motorId,ControllMsg.vel);
            break;
        case CurCfg:
            break;
        case MPC_Move:
...
void Bridge_PosUpdate()
{
    ...
    OSLIB_UART_SendData(&huart2,data_to_send,data_to_send_size*sizeof(uint8_t));
    if (cnt == 10)
    {
